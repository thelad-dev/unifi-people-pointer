...
# Presence/coordinator integration paths are still Phase 3 stubs.
pytestmark = [
    pytest.mark.integration,
    pytest.mark.skip(reason="Requires Phase 3 coordinator/presence implementation"),
]
...
class TestUniFiDownScenarios:
    ...
    async def test_unifi_controller_offline(self, hass, mock_config_entry):
        ...
        with patch.object(
            coordinator,
            "_fetch_clients",
            side_effect=ConnectionError("Controller offline"),
        ):
            await coordinator.async_refresh()

            assert coordinator.last_update_success is False
            assert (
                "offline" in coordinator.last_exception.lower()
                or "connection" in coordinator.last_exception.lower()
            )
    ...
    async def test_unifi_network_unreachable(self, hass, mock_config_entry):
        ...
        with patch.object(
            coordinator, "_fetch_clients", side_effect=OSError("Network is unreachable")
        ):
            await coordinator.async_refresh()
    ...
    async def test_unifi_firmware_upgrade_downtime(self, hass, mock_config_entry):
        ...
        with patch.object(
            coordinator, "_fetch_clients", side_effect=ConnectionError("Upgrading")
        ):
            await coordinator.async_refresh()
...
class TestFlappingWiFi:
    ...
    async def test_wifi_rapid_connect_disconnect(
        ...
        for i in range(5):
            ...
            state_changes.append(
                {"timestamp": timestamp, "state": state, "mac": "1c:3c:78:b8:ae:b5"}
            )
    ...
    async def test_wifi_poor_signal_flapping(
        self, hass, mock_config_entry, mock_unifi_api_clients_flapping
    ):
        ...
        # Poor signal client
        with patch.object(
            coordinator, "_fetch_clients", return_value=mock_unifi_api_clients_flapping
        ):
            await coordinator.async_refresh()
    ...
    async def test_wifi_flapping_dampening(
        self, hass, mock_config_entry, devices_data, people_data
    ):
        ...
        tracker = PresenceTracker(
            hass, devices_data["devices"], people_data["people"], debounce_seconds=30
        )
...
class TestUnknownMACs:
    ...
    async def test_unknown_mac_address(
        self, hass, mock_config_entry, mock_unifi_api_unknown_macs, devices_data
    ):
    ...
    async def test_unknown_mac_logging(
        self, hass, mock_config_entry, mock_unifi_api_unknown_macs
    ):
        ...
        with patch.object(
            coordinator, "_fetch_clients", return_value=mock_unifi_api_unknown_macs
        ):
    ...
    async def test_unknown_mac_manufacturer_hint(
        self, hass, manufacturers_data, mock_unifi_api_unknown_macs
    ):
    ...
    async def test_unknown_mac_with_known_oui(self, hass, manufacturers_data):
        ...
        client = {"mac": "38:7f:8b:aa:bb:cc", "hostname": "NewAppleDevice"}  # Apple OUI
    ...
    async def test_unknown_mac_guest_network(self, hass, mock_config_entry):
        ...
        coordinator = UniFiCoordinator(
            hass, mock_config_entry, track_guest_network=False
        )

        guest_clients = [
            {
                "mac": "aa:bb:cc:dd:ee:ff",
                "hostname": "GuestDevice",
                "essid": "Guest-Network",
            }
        ]
        ...
        with patch.object(coordinator, "_fetch_clients", return_value=guest_clients):
            ...
            assert (
                len(coordinator.data) == 0
                or coordinator.data[0]["essid"] != "Guest-Network"
            )
    ...
    async def test_unknown_mac_iot_device(self, hass, mock_config_entry):
        ...
        iot_clients = [{"mac": "aa:bb:cc:dd:ee:ff", "hostname": "smart-lightbulb-123"}]
...
class TestDuplicateMACs:
    ...
    async def test_duplicate_mac_different_aps(
        self, hass, mock_config_entry, mock_unifi_api_duplicate_macs
    ):
        """Test duplicate MAC appearing on different APs simultaneously."""
        from custom_components.unifi_people_pointer.api_parser import (
            deduplicate_clients,
        )
    ...
    async def test_duplicate_mac_keep_most_recent(
        self, hass, mock_config_entry, mock_unifi_api_duplicate_macs
    ):
    ...
    async def test_duplicate_mac_private_address_rotation(
        self, hass, mock_config_entry, devices_data
    ):
