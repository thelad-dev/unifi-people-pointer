...
class TestFallbackLogic:
    ...
    async def test_fallback_to_cached_state(self, hass, mock_config_entry):
        ...
        mock_clients = [
            {"mac": "1c:3c:78:b8:ae:b5", "hostname": "iPhone-JD", "last_seen": datetime.now().timestamp()}
        ]
        ...
        with patch.object(coordinator, '_fetch_clients', side_effect=ConnectionError("API unavailable")):
            await coordinator.async_refresh()
    ...
    async def test_fallback_cache_expiry(self, hass, mock_config_entry):
        ...
        coordinator = UniFiCoordinator(hass, mock_config_entry, cache_ttl=300)  # 5 min TTL
        
        # Set old cached data
        old_timestamp = datetime.now() - timedelta(minutes=10)
        coordinator._cache_timestamp = old_timestamp
        coordinator._cached_data = [{"mac": "aa:bb:cc:dd:ee:ff", "hostname": "OldDevice"}]
    ...
    async def test_fallback_to_ssh_mongodb(self, hass, mock_config_entry):
        ...
        coordinator = UniFiCoordinator(hass, mock_config_entry, enable_ssh_fallback=True)
        ...
        with patch.object(coordinator, '_fetch_clients', side_effect=ConnectionError("API unavailable")):
            with patch.object(coordinator, '_fetch_via_ssh', return_value=mock_ssh_clients):
                await coordinator.async_refresh()
    ...
    async def test_fallback_ssh_also_fails(self, hass, mock_config_entry):
        ...
        with patch.object(coordinator, '_fetch_clients', side_effect=ConnectionError("API unavailable")):
            with patch.object(coordinator, '_fetch_via_ssh', side_effect=ConnectionError("SSH unavailable")):
                await coordinator.async_refresh()
                
                # Should use empty data or last known good
                assert coordinator.data is not None or coordinator.last_update_success is False

    async def test_fallback_to_last_known_good(self, hass, mock_config_entry, devices_data, people_data):
    ...
    async def test_fallback_circuit_breaker(self, hass, mock_config_entry):
        ...
        coordinator = UniFiCoordinator(hass, mock_config_entry, circuit_breaker_threshold=5)
    ...
    async def test_fallback_state_persistence(self, hass, mock_config_entry):
        ...
        with patch.object(coordinator, '_fetch_clients', return_value=[
            {"mac": "1c:3c:78:b8:ae:b5", "hostname": "iPhone-JD"}
        ]):
            await coordinator.async_refresh()
        ...
        with patch.object(coordinator_new, '_fetch_clients', side_effect=ConnectionError("API unavailable")):
            await coordinator_new.async_refresh()
            
            # Should have restored state
            assert coordinator_new.data is not None or coordinator_new._last_known_good is not None
