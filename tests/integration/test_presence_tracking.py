...
# Presence tracking helpers are still Phase 3 stubs.
pytestmark = [
    pytest.mark.integration,
    pytest.mark.skip(reason="Requires Phase 3 presence implementation"),
]
...
class TestPresenceTracking:
    ...
    async def test_person_home_single_device(self, hass, devices_data, people_data):
        ...
        presence = determine_presence(
            people_data["people"], devices_data["devices"], clients
        )
    ...
    async def test_person_multiple_devices_all_online(
        self, hass, devices_data, people_data
    ):
    ...
    async def test_person_untracked_device_ignored(
        self, hass, devices_data, people_data
    ):
        ...
        clients = [{"mac": "02:a2:54:a8:e1:98", "hostname": "iPhone"}]
    ...
    async def test_presence_state_entity_update(self, hass, mock_config_entry):
        ...
        with patch(
            "custom_components.unifi_people_pointer.coordinator.UniFiCoordinator._fetch_clients",
    ...
    async def test_presence_transition_home_to_away(
        self, hass, devices_data, people_data
    ):
    ...
    async def test_presence_transition_away_to_home(
        self, hass, devices_data, people_data
    ):
    ...
    async def test_presence_with_hostname_match_only(
        self, hass, devices_data, people_data
    ):
        ...
        clients = [{"mac": "aa:bb:cc:dd:ee:ff", "hostname": "iPhone-JD"}]
    ...
    async def test_presence_attributes(self, hass, mock_config_entry):
        ...
        with patch(
            "custom_components.unifi_people_pointer.coordinator.UniFiCoordinator._fetch_clients",
            return_value=[
                {"mac": "1c:3c:78:b8:ae:b5", "hostname": "iPhone-JD", "signal": -45}
            ],
        ):
            await async_setup_entry(hass, mock_config_entry)
            await hass.async_block_till_done()
    ...
    async def test_presence_history_tracking(self, hass, devices_data, people_data):
        ...
        tracker = PresenceTracker(
            hass, devices_data["devices"], people_data["people"], track_history=True
        )
...
class TestPresenceConfidenceScore:
    ...
    async def test_confidence_high_recent_seen(self, hass):
        ...
        confidence = calculate_confidence(
            last_seen=recent_seen, signal_strength=-40, is_private_mac=False
        )
    ...
    async def test_confidence_low_old_seen(self, hass):
        ...
        confidence = calculate_confidence(
            last_seen=old_seen, signal_strength=-40, is_private_mac=False
        )
    ...
    async def test_confidence_medium_private_mac(self, hass):
        ...
        confidence = calculate_confidence(
            last_seen=recent_seen, signal_strength=-40, is_private_mac=True
        )
